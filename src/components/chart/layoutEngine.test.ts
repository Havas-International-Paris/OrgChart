...
describe('layoutWithDagre', () => {
  it('returns top-left positions, not dagre’s centres', () => {
    const { nodes, edges } = build(['solo'], []);
    const [laid] = layoutWithDagre(nodes, edges);
    // dagre centres a lone node at (width/2, height/2), so the top-left
    // corner lands exactly on the origin.
    expect(laid.position.x).toBeCloseTo(0);
    expect(laid.position.y).toBeCloseTo(0);
  });

  it('preserves every node and its other properties', () => {
  ...
  it('places a report exactly one rank below its manager', () => {
    const { nodes, edges } = build(['m', 'e'], [['m', 'e']]);
    const pos = byId(layoutWithDagre(nodes, edges));
    expect(pos.get('e')!.y - pos.get('m')!.y).toBeCloseTo(NODE_HEIGHT + RANK_SEP);
  });

  it('puts siblings on the same rank, side by side', () => {
  ...
  it('leaves dagre’s own order alone when no sibling order is supplied', () => {
    const { nodes, edges } = build(['m', 'p', 'q'], [['m', 'p'], ['m', 'q']]);
    const withoutArg = byId(layoutWithDagre(nodes, edges));
    const allNull = byId(layoutWithDagre(nodes, edges, () => null));
    expect(allNull.get('p')).toEqual(withoutArg.get('p'));
    expect(allNull.get('q')).toEqual(withoutArg.get('q'));
  });

  it('honours an explicit sibling order in both directions', () => {
    const { nodes, edges } = build(['m', 'p', 'q'], [['m', 'p'], ['m', 'q']]);

    const qFirst = byId(layoutWithDagre(nodes, edges, order({ p: 2000, q: 1000 })));
    expect(qFirst.get('q')!.x).toBeLessThan(qFirst.get('p')!.x);

    const pFirst = byId(layoutWithDagre(nodes, edges, order({ p: 1000, q: 2000 })));
    expect(pFirst.get('p')!.x).toBeLessThan(pFirst.get('q')!.x);
  });

  it('orders roots too, not just children', () => {
    const { nodes, edges } = build(['r1', 'r2'], []);
    const pos = byId(layoutWithDagre(nodes, edges, order({ r1: 2000, r2: 1000 })));
    expect(pos.get('r2')!.x).toBeLessThan(pos.get('r1')!.x);
  });

  // The backfill-on-first-touch invariant: a group is either fully ordered or
  // treated as untouched. A half-ordered group must fall back to dagre rather
  // than packing the ordered members and stranding the rest.
  it('ignores a group where only some siblings have an order', () => {
    const { nodes, edges } = build(['m', 'p', 'q'], [['m', 'p'], ['m', 'q']]);
    const baseline = byId(layoutWithDagre(nodes, edges));
    const partial = byId(layoutWithDagre(nodes, edges, order({ p: 1000 })));
    expect(partial.get('p')).toEqual(baseline.get('p'));
    expect(partial.get('q')).toEqual(baseline.get('q'));
  });

  it('leaves an only child where dagre put it', () => {
    const { nodes, edges } = build(['m', 'only'], [['m', 'only']]);
    const baseline = byId(layoutWithDagre(nodes, edges));
    const ordered = byId(layoutWithDagre(nodes, edges, order({ only: 1000 })));
    expect(ordered.get('only')).toEqual(baseline.get('only'));
  });

  it('drags a reordered manager’s whole subtree along with them', () => {
    ...
    const before = byId(layoutWithDagre(nodes, edges, order({ boss: 1000, solo: 2000 })));
    const after = byId(layoutWithDagre(nodes, edges, order({ boss: 2000, solo: 1000 })));
  ...
  // siblings into evenly-sized slots, or permuting dagre's original slot
  // positions, both assume similar subtree widths. Moving a wide subtree into
  // a gap sized for a narrow one made its descendants collide with the
  // neighbouring subtree. Positions must be packed by each subtree's real
  // measured width instead.
  it('never overlaps sibling subtrees of very different widths', () => {
    const { nodes, edges } = build(
    ...
    const laid = layoutWithDagre(nodes, edges, order({ n1: 1000, wide: 2000, n2: 3000 }));
  ...
  it('keeps subtrees apart whichever slot the wide one is moved into', () => {
    const { nodes, edges } = build(
      ['m', 'wide', 'n1', 'n2', 'w1', 'w2', 'w3', 'w4'],
      [
        ['m', 'wide'],
    ...
    for (const arrangement of arrangements) {
      const laid = layoutWithDagre(nodes, edges, order(arrangement));
  ...
  it('keeps a reordered group roughly where it was, rather than drifting sideways', () => {
    const { nodes, edges } = build(['m', 'p', 'q', 'r'], [['m', 'p'], ['m', 'q'], ['m', 'r']]);
    const centreOf = (laid: Node[]) =>
      ['p', 'q', 'r'].reduce((sum, id) => sum + byId(laid).get(id)!.x, 0) / 3;

    const before = centreOf(layoutWithDagre(nodes, edges, order({ p: 1000, q: 2000, r: 3000 })));
    const after = centreOf(layoutWithDagre(nodes, edges, order({ p: 3000, q: 1000, r: 2000 })));
    expect(after).toBeCloseTo(before);
  });

  it('handles an empty chart', () => {
    expect(layoutWithDagre([], [])).toEqual([]);
  });
});
