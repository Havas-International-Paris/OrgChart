...
describe('computeReorder', () => {
  ...
  it('backfills hidden siblings too, so the group never ends up half-ordered', () => {
    ...
    // geometrically, yet it must still receive a value. Dropped just past `b`
    // (not exactly on it) so the drag actually crosses its rank.
    const g = geometry({
      x: { boss: 250, a: 0, b: 250, hidden: 500 },
      parent: { a: 'boss', b: 'boss', hidden: 'boss' },
      hidden: ['hidden'],
    });
    const outcome = computeReorder(g, 'a', 260);
    if (outcome.kind !== 'reorder') throw new Error('expected a reorder');
  ...
  it('orders roots against each other, not just children', () => {
    // Dropped past (not exactly on) r2 so the drag actually crosses its rank.
    const g = geometry({ x: { r1: 0, r2: 300 } });
    expect(computeReorder(g, 'r1', 310).kind).toBe('reorder');
  });
  ...
  it('ignores siblings from other managers', () => {
    ...
    // from a different cluster entirely. Dropped just past `b` so the drag
    // actually crosses its rank.
    const g = geometry({
  ...
  it('is idempotent: dropping a card back exactly where it already is snaps back', () => {
    // No rank change, so this must agree with findDisplacementTargets
    // highlighting nobody at this exact position — a real DB write here
    // would be a silent no-op undo entry for nothing.
    const g = geometry({ ...trio(), order: { a: 1000, b: 2000, c: 3000 } });
    expect(computeReorder(g, 'b', 250)).toEqual({ kind: 'snap-back' });
  });
});
...
describe('findDisplacementTargets', () => {
  ...
  it('highlights nothing before the dragged card covers roughly half of a sibling', () => {
    // `a` (starting at 0) needs to reach 140 before it covers half of `b`
    // (at 250, NODE_WIDTH 220 wide) — well short of that here.
    const g = geometry(trio());
    expect(findDisplacementTargets(g, 'a', 100).size).toBe(0);
  });

  it('highlights a sibling once roughly half its card is covered — not once fully passed', () => {
    // 150 is well short of `b`'s own position (250) — under a full-crossing
    // threshold this would highlight nothing. It's past the 50%-overlap
    // point (140), so `b` must already be highlighted.
    const g = geometry(trio());
    expect([...findDisplacementTargets(g, 'a', 150)]).toEqual(['b']);
  });

  it('highlights every sibling crossed in one drag, not just the nearest', () => {
    // Past both `b` (250) and `c` (500) in a single drag — both changed rank.
    const g = geometry(trio());
    expect([...findDisplacementTargets(g, 'a', 500 + NODE_WIDTH - 10)].sort()).toEqual(['b', 'c']);
  });
  ...
  it('includes the displaced sibling’s whole subtree, not just their card', () => {
    ...
    expect([...findDisplacementTargets(g, 'a', 260)].sort()).toEqual(['b', 'b1', 'b1a', 'b2']);
  });

  it('never disagrees with computeReorder: a highlighted drop always reorders, an unhighlighted one never does', () => {
    // Both share isWithinGroupSpan AND the crossing threshold now, so a drop
    // can't confirm anything the live feedback hadn't already promised.
    // Sweep across and past the span.
    const g = geometry(trio());
