...
export function useReportingChain(
  ...
  return useMemo(
    () => computeReportingChain(activeId, relationships, childrenOf),
    [activeId, relationships, childrenOf],
  );
}

// The hook's whole body, extracted as a plain function so it can be tested
// without mounting React — see computeVisibleGraph in useVisibleGraph.ts for
// the same split and the reasoning behind it.
export function computeReportingChain(
  activeId: string | null,
  relationships: ReportingRelationship[],
  childrenOf: Map<string, string[]>,
): ReportingChainResult {
