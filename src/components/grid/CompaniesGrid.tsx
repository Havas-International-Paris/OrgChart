import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { useTranslation } from 'react-i18next';
import { useCompanies } from '../../hooks/useCompanies';
import { companyColorMap } from '../../lib/companyColor';
import { useUiPreferencesStore } from '../../stores/uiPreferencesStore';
import { EditableCell } from './EditableCell';
import { useEditableRows } from './useEditableRows';
import { compareValues, densityClasses, effectiveForSort, type FieldKind } from './editableGridLogic';
import type { Company } from '../../types/domain';

const FIELDS = ['name'] as const;
type Field = (typeof FIELDS)[number];
const FIELD_KIND: Record<Field, FieldKind> = { name: 'text' };

function emptyDraft(): Company {
  return { id: `draft-${crypto.randomUUID()}`, name: '', color: null, created_at: new Date().toISOString() };
}

// See DepartmentsGrid.tsx's ColorSwatch for why this binds a native `change`
// listener via a ref rather than React's onChange prop.
function ColorSwatch({ color, title, onCommit }: { color: string; title: string; onCommit: (color: string) => void }) {
  const inputRef = useRef<HTMLInputElement>(null);
  const onCommitRef = useRef(onCommit);
  onCommitRef.current = onCommit;

  useEffect(() => {
    const el = inputRef.current;
    if (!el) return;
    const handleChange = () => onCommitRef.current(el.value);
    el.addEventListener('change', handleChange);
    return () => el.removeEventListener('change', handleChange);
  }, []);

  return (
    <label
      className="relative block h-3 w-3 cursor-pointer rounded-full"
      style={{ backgroundColor: color }}
      title={title}
    >
      <input
        key={color}
        ref={inputRef}
        type="color"
        defaultValue={color}
        className="absolute h-px w-px overflow-hidden opacity-0"
      />
    </label>
  );
}

export function CompaniesGrid() {
  const { t } = useTranslation();
  const { companies, loading, error, createCompany, updateCompany, updateCompanyColor, deleteCompany } =
    useCompanies();
  const gridDensity = useUiPreferencesStore((s) => s.gridDensity);
  const [actionError, setActionError] = useState<string | null>(null);
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('asc');

  const editing = useEditableRows<Company, Field>({
    fields: FIELDS,
    fieldKind: FIELD_KIND,
    requiredFields: FIELDS,
    createRow: useCallback((draft: Company) => createCompany(draft.name), [createCompany]),
    updateField: useCallback(
      async (row: Company, _field: Field, value: string | null) => {
        await updateCompany(row.id, value ?? '', row.name);
      },
      [updateCompany],
    ),
  });

  const colorByName = useMemo(() => companyColorMap(companies), [companies]);

  const handleDelete = useCallback(
    (id: string) => {
      setActionError(null);
      deleteCompany(id).catch((err) => setActionError(err instanceof Error ? err.message : String(err)));
    },
    [deleteCompany],
  );

  const sorted = useMemo(() => {
    const rows = [...companies].sort((a, b) => {
      const cmp = compareValues(
        effectiveForSort(a, editing.frozenRow).name,
        effectiveForSort(b, editing.frozenRow).name,
      );
      return sortDir === 'asc' ? cmp : -cmp;
    });
    return rows;
  }, [companies, sortDir, editing.frozenRow]);

  const { rowPad, cellText } = densityClasses(gridDensity);
  const rows = editing.draft ? [editing.draft, ...sorted] : sorted;

  return (
    <div className="flex h-full flex-col gap-2">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-slate-700">{t('grid.companies.title')}</h2>
        <button
          onClick={() => editing.startDraft(emptyDraft())}
          disabled={Boolean(editing.draft)}
          className="rounded bg-slate-900 px-3 py-1 text-xs font-medium text-white disabled:opacity-50"
        >
          {t('grid.companies.add')}
        </button>
      </div>
      {(error || actionError) && <p className="text-sm text-red-600">{error ?? actionError}</p>}
      <div className="min-h-0 flex-1 overflow-auto">
        <table role="grid" className={`w-full border-collapse ${cellText}`}>
          <thead>
            <tr role="row" className="border-b border-slate-200 text-left text-slate-500">
              <th className="w-14 px-2 font-medium">{t('grid.companies.colorHeader')}</th>
              <th className={`${rowPad} px-2 font-medium`}>
                <button
                  type="button"
                  onClick={() => setSortDir((dir) => (dir === 'asc' ? 'desc' : 'asc'))}
                  className="flex items-center gap-1 hover:text-slate-800"
                >
                  {t('grid.companies.nameHeader')}
                  <span className="text-[10px]">{sortDir === 'asc' ? '▲' : '▼'}</span>
                </button>
              </th>
              <th className="w-10" />
            </tr>
          </thead>
          <tbody>
            {loading && (
              <tr role="row">
                <td colSpan={3} className="p-4 text-center text-slate-400">
                  {t('grid.companies.loading')}
                </td>
              </tr>
            )}
            {!loading && rows.length === 0 && (
              <tr role="row">
                <td colSpan={3} className="p-4 text-center text-slate-400">
                  {t('grid.companies.empty')}
                </td>
              </tr>
            )}
            {rows.map((row) => {
              const color = colorByName.get(row.name);
              const isDraftRow = row.id === editing.draft?.id;
              return (
                <tr key={row.id} role="row" className="border-b border-slate-100 hover:bg-slate-50">
                  <td role="gridcell" className={`${rowPad} px-2`}>
                    {color &&
                      (isDraftRow ? (
                        <span className="block h-3 w-3 rounded-full" style={{ backgroundColor: color }} title={color} />
                      ) : (
                        <ColorSwatch
                          color={color}
                          title={t('grid.companies.changeColor', { name: row.name })}
                          onCommit={(newColor) => updateCompanyColor(row.id, newColor)}
                        />
                      ))}
                  </td>
                  <td role="gridcell" className={`${rowPad} px-2`}>
                    <EditableCell editing={editing} row={row} field="name" title={t('grid.companies.editName')} />
                  </td>
                  <td role="gridcell" className={`${rowPad} px-2`}>
                    {row.id !== editing.draft?.id && (
                      <button
                        onClick={() => handleDelete(row.id)}
                        title={t('grid.companies.delete')}
                        className="text-slate-400 hover:text-red-600"
                      >
                        ✕
                      </button>
                    )}
                  </td>
                </tr>
              );
            })}
          </tbody>
        </table>
      </div>
    </div>
  );
}
