...
**If `~/Downloads` isn't reachable from this session** (a background/sandboxed
job often can't read the user's Downloads folder at all — `EPERM` from every
tool, not just Bash), skip the local-disk hop entirely: upload the blob
straight from the image page to Supabase Storage instead of downloading it.
`media.licdn.com` carries no restrictive CSP (it's a bare asset host, unlike
`linkedin.com` itself), so a cross-origin `fetch` POST from that page to
`https://<project>.supabase.co/storage/v1/object/employee-photos/<path>`
works directly:

```js
var blob = await fetch(location.href).then(r => r.blob());
await fetch('https://<project>.supabase.co/storage/v1/object/employee-photos/<employee_id>/<uuid>.jpg', {
  method: 'POST',
  headers: { apikey: '<anon key>', Authorization: 'Bearer <user access_token>', 'Content-Type': 'image/jpeg' },
  body: blob,
});
```

Get the `access_token` beforehand from the shell (not the page) via
`POST {SUPABASE_URL}/auth/v1/token?grant_type=password` with the project's
E2E test credentials (`.env.test.local`) and the anon key as `apikey` — same
credentials `scripts/_upload_photos.mjs` already uses through `supabase-js`,
just called over plain REST instead since there's no Node process in the
browser. Then update the employee row (`photo_path`, and reset
`photo_zoom`/`photo_pan_x`/`photo_pan_y` to defaults) the normal way. Verified
working end-to-end 2026-09-04.
